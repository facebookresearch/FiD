...
class T5Attention(nn.Module):
    ...
    def forward(self, input, mask=None, kv=None, position_bias=None, cache=None, head_mask=None):
        ...
        if mask is not None:
            position_bias_mask = position_bias + mask
...
class T5PreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        elif isinstance(module, (T5Model, T5MergeForConditionalGeneration)):
...
class T5Stack(T5PreTrainedModel):
    ...
    def forward(
        ...
        num_blocks = len(self.block) if apply_up_to is None else apply_up_to
...
class T5MergeForConditionalGeneration(T5PreTrainedModel):
    ...
    Examples::
        ...
        """
        if encoder_outputs is None:
            hidden_states, attention_mask, context_ids = self.apply_encoder_list(context_ids, context_mask)
        else:
            hidden_states = encoder_outputs[0]
        ...
        if hasattr(self, 'extra_len') and self.extra_len > 0:
            sequence_output = sequence_output[:, self.extra_len:]
        ...
        if lm_labels is not None:
            ...
            #loss = loss_fct(p.view(-1, p.size(-1)), lm_labels.view(-1))
            decoder_outputs = (
                loss,
    ...
    def generate(
        ...
        if self.config.is_encoder_decoder:
            ...
            # Encode context
            encoder_outputs, attention_mask, context_ids = self.apply_encoder_list(context_ids, context_mask)
            encoder_outputs = (encoder_outputs, )
            context_outputs = None
    ...
    def _generate_no_beam_search(
        ...
        while cur_len < max_length:
            ...
            init_input_ids = input_ids.clone()

            model_inputs = self.prepare_inputs_for_generation(input_ids, past=past, attention_mask=attention_mask)

            outputs = self(**model_inputs, context_outputs=context_outputs, context_mask=context_mask, context_ids=context_ids)
